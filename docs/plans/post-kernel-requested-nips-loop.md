...
| `92` | not started | `noztr` | bounded `imeta` parse/build/validate, URL-to-content matching helpers, NIP-94 field reuse | media fetch, preview generation, upload flow, attachment UX | kernel-medium |
| `94` | not started | `noztr` | bounded file-metadata event parse/build/validate, typed URL/MIME/hash/size/dim handling | file upload/download flow, preview fetch, hosting service integration | kernel-low |
...
| `99` | not started | `noztr` | bounded listing metadata parse/build/validate, draft/inactive distinction, price/status tag handling | listing publish UX, search, inventory/state workflows, commerce extensions | kernel-medium |
| `B0` | not started | `noztr` | bounded web-bookmark parse/build helpers around `39701`, required `d`, optional metadata and hashtag handling | bookmark sync, browser integration, preview/render policy | kernel-low |
