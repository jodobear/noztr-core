...
| `06` mnemonic / derivation | mnemonic validate, seed derivation, canonical key derivation, bounded BIP-85 hex/BIP39 derivation, zeroization, typed errors | wallet UX, account selection, secret storage, import/export flows | wallet/account manager over kernel derivation helpers | current split is correct; full NFKD remains the only significant deferred kernel follow-up |
