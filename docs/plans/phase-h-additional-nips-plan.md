...
| 17 | expansion-complete | Bounded private-message helpers are now implemented for kind-14 message parsing, kind-15 file-message parsing, direct wrap-to-rumor reuse of the existing NIP-59/NIP-44 trust boundary for both message kinds, kind-10050 relay-list extraction, and direct `p`/`relay` builders. |
...
| 26 | expansion-complete | Bounded delegation helpers are now implemented for exact `delegation` tag parse/build, fixed-capacity condition parsing/formatting, exact delegation message construction, deterministic Schnorr sign/verify, and pure event-condition validation. Rust and TypeScript evidence are source-review-only because neither active reference lane exposes a dedicated NIP-26 helper surface. |
| 37 | expansion-complete | Bounded NIP-37 helpers are now implemented for kind-`31234` draft-wrap metadata parsing, validated draft JSON NIP-44 encrypt/decrypt helpers, direct `d`/`k`/`expiration` tag builders, kind-`10013` private relay-tag JSON serialization/extraction, and NIP-44 private relay-list extraction. Rust and TypeScript evidence are source-review-only because neither active reference lane exposes a dedicated NIP-37 helper surface. |
| 58 | expansion-complete | Bounded badge helpers are now implemented for kind-`30009` definition extraction/building, kind-`8` award extraction/building, kind-`30008` profile-badge pair extraction, and pure badge/award/profile consistency validation. |
| 84 | expansion-complete | Bounded highlight helpers are now implemented for kind-`9802` source extraction/building across `e`/`a`/`r`, bounded `p` attribution and `r` URL-reference extraction/building, and optional `context` / `comment` handling. |
...
| 05 | expansion-complete | Bounded NIP-05 identity helpers are now implemented for spec-shaped address parsing, bare-domain `_` canonicalization, canonical well-known URL composition, raw `nostr.json` verification, and bounded optional `relays` / `nip46` extraction keyed by the matched public key. |
...
| 36 | expansion-complete | Bounded content-warning helpers are now implemented for exact `content-warning` tag extraction/building, accepted empty-reason normalization, and direct NIP-32 `content-warning` namespace/label builders and matchers. |
| 56 | expansion-complete | Bounded reporting helpers are now implemented for kind-`1984` report extraction/building, required `p` target presence, typed report enums, optional `e`/`x` report targets, bounded `server` tag extraction, and tolerant ignoring of clearly generic `e` forms. |
...
- Complete:
  - `06`
    ...
    - current acceptance evidence:
      - official BIP39 mnemonic-to-seed vectors are covered
      - both official NIP-06 vectors from `docs/nips/06.md` are covered
