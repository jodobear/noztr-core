...
| 03 | Preserve exact attestation target references, bounded proof decoding, and honest verification claims; review proof-shape strictness so we reject malformed `1040` events without pretending to perform deeper OpenTimestamps / Bitcoin verification that the kernel does not actually implement. |
...
| 29 | Preserve bounded relay-generated event parsing and deterministic group-state tags without embedding load/fetch/subscription logic in the kernel; review compatibility shims such as `public`/`open` metadata aliases only where deployed helper behavior makes them materially useful. |
