...
| 73 | complete | `HARNESS_COVERED BASELINE PASS` | `NOT_COVERED_IN_THIS_PASS` | No Layer 1 change required in this pass; current external-id parsing remains intentionally broader than the NIP normalization table pending stronger ecosystem evidence | none | none | The NIP text specifies normalized forms for several IDs, but the strongest production reference lane is also broad here; tightening now would be a compatibility-policy change rather than a clear bug fix, so the kernel keeps the current bounded parser for now |
...
- NIP-73: no Layer 1 change required in this pass; keep the current broader external-id parser
  until stronger ecosystem evidence justifies enforcing the stricter normalization table.
