...
| 29 | complete | `SOURCE_REVIEW_ONLY no dedicated rust-nostr helper surface` | `HARNESS_COVERED BASELINE PASS` | Accepted deployed three-slot `h` tags with optional relay hints and stopped rejecting the empty compatibility label slot emitted by `nostr-tools` on admin `p` tags | none | none | The NIP text only requires the `h` value, but deployed group tooling carries an extra relay hint; admin-label modeling remains intentionally out of scope, so the kernel ignores only the empty compatibility slot and keeps canonical builders raw |
...
- NIP-29: accept deployed three-slot `h` tags with optional relay hints and ignore the empty admin
  compatibility label emitted by `nostr-tools`, while keeping canonical builders on the raw
  group-id and pubkey-plus-roles shapes.
